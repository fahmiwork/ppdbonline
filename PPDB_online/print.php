...
<body>
  ...
  if(empty($tgl_awal) or empty($tgl_akhir)){ // Cek jika tgl_awal atau tgl_akhir kosong, maka :
    ...
    $query = "SELECT * FROM tb_pendaftar WHERE status_pendaftaran='Diterima' ORDER BY tgl ASC";
    $query2 = "SELECT min(tgl) AS tgl_kecil, max(tgl) AS tgl_besar FROM tb_pendaftar WHERE status_pendaftaran='Diterima'";
  ...
  <table class="table" border="1" width="100%" style="margin-top: 10px;">
    ...
    if($row > 0){ // Jika jumlah data lebih dari 0 (Berarti jika data ada)
      ...
      while($data = mysqli_fetch_array($sql)){ // Ambil semua data dari hasil eksekusi $sql
        ...
        echo "<td class='txtc'>".$data['asal_sekolah']."</td>";
        echo "</tr>";
      }
...
$pdf->WriteHTML($html);
$pdf->Output('Data Pendaftar Diterima.pdf', 'I');
?>